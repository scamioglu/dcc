...
// ── ICE / TURN sunucu bilgileri ────────────────────────────────────────────────
// Client bu endpoint'i çağırarak zaman sınırlı TURN credential'ı alır.
// Hardcoded credential güvenlik açığıdır; sunucudan dinamik olarak alınmalıdır.
app.get("/api/ice-servers", async (req, res) => {
  const meteredKey = process.env.METERED_API_KEY;

  // Metered.ca API key varsa → gerçek, zaman sınırlı TURN credential'ı al
  if (meteredKey) {
    try {
      const r = await fetch(
        `https://nexus.metered.ca/api/v1/turn/credentials?apiKey=${meteredKey}`
      );
      if (!r.ok) throw new Error(`Metered API ${r.status}`);
      const iceServers = await r.json();
      return res.json({ iceServers });
    } catch (err) {
      console.error("[ICE] Metered API hatası:", err.message);
    }
  }

  // Env var ile elle tanımlanmış TURN sunucu varsa onu kullan
  ...
  // Hiçbir TURN ayarı yoksa sadece STUN döndür (aynı ağda çalışır, farklı ağda çalışmaz)
  console.warn("[ICE] TURN sunucu ayarlanmamış. Farklı ağlarda bağlantı kurulamayabilir.");
  res.json({
    iceServers: [
      { urls: "stun:stun.l.google.com:19302" },
      { urls: "stun:stun1.l.google.com:19302" },
...
io.on("connection", (socket) => {
  ...
  socket.on("voice:join", ({ channelId }) => {
    ...
    if (user.currentVoice && user.currentVoice !== channelId) {
      leaveVoice(socket, user.currentVoice);
    }

    // Aynı kanalda zaten varsa yeniden ekleme — hayalet üye koruması
    if (user.currentVoice === channelId && voiceRooms[channelId]?.[socket.id]) {
      // Sadece existing-peers bilgisini yenile
      const existingPeers = Object.keys(voiceRooms[channelId]).filter((id) => id !== socket.id);
...
function leaveVoice(socket, channelId) {
  ...
  const remaining = Object.values(room);

  socket.to(`voice:${channelId}`).emit("voice:peer-left", { peerId: socket.id });

  /* KRITIK: socket.leave'den ÖNCE emit et — sonra emit edilirse
     kullanıcı artık o odada değil ve güncellemeyi alamıyor → sidebar hayalet üye */
  socket.emit("voice:members-update", { channelId, members: remaining }); // çıkan kullanıcıya
  io.to(`voice:${channelId}`).emit("voice:members-update", { channelId, members: remaining }); // kalan üyelere
