...
export default function Sidebar({ user, socket, activeTextChannel, activeVoiceChannel,
  onSelectTextChannel, onSelectVoiceChannel, onlineUsers, isMobilePanel }) {
  ...
  return (
    <div className={`sidebar ${isMobilePanel ? "sidebar-mobile-panel" : ""}`}>
      ...
      <div className="sidebar-scroll">
        ...
        <div className="sidebar-section-label" style={{ marginTop:12 }}>Sesli Kanallar</div>
          ...
          return (
            <div key={ch.id}>
              <button
                ...
                onClick={() => onSelectVoiceChannel(isActive ? null : ch.id)}>
                <span className="ch-voice">🔊</span>
                <span className="ch-name">{ch.name}</span>
                {members.length > 0 && <span className="ch-count">{members.length}</span>}
                ...
                <div className="vcm-list">
                  {members.map((m) => (
                    <div key={m.socketId} className="vcm-item">
                      <div className="vcm-avatar" style={{ background: avatarColor(m.username) }}>
                        {m.username?.slice(0,2).toUpperCase()}
                      </div>
                      <span className="vcm-name">{m.username}{m.username===user.username&&" (sen)"}</span>
