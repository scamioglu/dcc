  ...
  useEffect(() => {
    ...
    const tryPlay = async () => {
      try {
        ...
        console.warn("Audio autoplay engellendi:", e);
      }
    };
    ...
    pc.ontrack = ({ track, streams }) => {
      ...
      if (streams && streams[0]) {
        streams[0].getTracks().forEach((t) => {
          const exists = ms.getTracks().find((x) => x.id === t.id);
          if (!exists) ms.addTrack(t);
        });
      }
      ...
      Object.values(peersRef.current).forEach((pc) => {
        ...
        try {
          ...
          socket?.emit("webrtc:offer", {
            to: Object.keys(peersRef.current).find(
              (k) => peersRef.current[k] === pc
            ),
            offer,
            channelId,
          });
        } catch (e) {
          console.warn("Renegotiation hatası:", e);
        }
      });
    });
