...
/* Discord benzeri katılma / ayrılma bildirim sesleri — Web Audio API */
function playNotif(type) {
  try {
    ...
    if (type === "join") {
      tone(523, 784,  t,        0.14);  // C5 → G5
      tone(784, 1047, t + 0.15, 0.14); // G5 → C6
    } else {
      tone(784, 523, t,        0.10);   // G5 → C5
      tone(523, 392, t + 0.11, 0.14);  // C5 → G4
    }
    setTimeout(() => ctx.close(), 1500);
  } catch { /* ses yoksa sessizce geç */ }
}
...
export default function VoiceRoom({ user, socket, channelId, onLeave }) {
  ...
  const audioCtxRef = useRef(null);  // Web Audio context — cleanup'ta kapatılır

  /* 1. Mikrofon — Web Audio zinciri: HighPass → Kompresör → Gain
     Bu zincir browser constraint'larının ötesinde ortam sesini bastırır ve sesi yükseltir */
  useEffect(() => {
    let rawStream = null;
    ...
    navigator.mediaDevices.getUserMedia({
      audio: {
        ...
        autoGainControl:          false,  // AGC ortam sesini yükseltir — kapalı
        sampleRate:               48000,
        channelCount:             1,      // mono: gürültü bastırma mono'da çok daha etkili
        voiceIsolation:           true,   // Chrome 116+ AI ses izolasyonu
    ...
    .then((stream) => {
      ...
      /* ── Web Audio işleme zinciri ── */
      const ctx = new (window.AudioContext || window.webkitAudioContext)({ sampleRate: 48000 });
      audioCtxRef.current = ctx;
      const src = ctx.createMediaStreamSource(stream);

      /* 1) Yüksek geçiş filtresi — 120 Hz altı: klima, fan, ortam uğultusu */
      const hiPass = ctx.createBiquadFilter();
      hiPass.type = "highpass";
      hiPass.frequency.value = 120;
      hiPass.Q.value = 0.8;

      /* 2) Dinamik kompresör — sessiz ortam sesini bastır, konuşma geçsin
            threshold düşük + ratio yüksek = agresif gürültü kapısı gibi davranır */
      const comp = ctx.createDynamicsCompressor();
      comp.threshold.value = -42;  // dB — bu değerin altı bastırılır
      comp.knee.value      = 12;
      comp.ratio.value     = 15;   // yüksek oran = daha sert bastırma
      comp.attack.value    = 0.003;
      comp.release.value   = 0.12;

      /* 3) Gain — sesi yükselt (~+8 dB) */
      const gain = ctx.createGain();
      gain.gain.value = 2.5;

      /* Zinciri bağla ve yeni stream oluştur */
  ...
  /* cleanup — bileşen unmount'ta kaynaları serbest bırak ve kanaldan çık */
  useEffect(() => {
    ...
    return () => {
      ...
      /* KRITIK: unmount'ta voice:leave emit edilmezse sunucu hayalet üye görür */
      currentSocket?.emit("voice:leave", { channelId: currentChannelId });
    };
  }, [socket, channelId]);
...
pc.onconnectionstatechange = () => {
  ...
  const startScreenShare = async () => {
    try {
      /* getDisplayMedia için sadece ideal kullan — min/max ve latencyHint
         video constraint olarak desteklenmiyor, OverconstrainedError fırlatır */
      const stream = await navigator.mediaDevices.getDisplayMedia({
        video: {
          width:          { ideal: 1920 },
          height:         { ideal: 1080 },
          frameRate:      { ideal: 60, max: 60 },  // 60fps akıcılık, bitrate ayarı kaliteyi korur
          cursor:         "always",
        ...
        audio: {
          ...
          sampleRate:        48000,
          // suppressLocalAudioPlayback KALDIRILDI — açık bırakılınca paylaşan PC'de ses kesiliyordu
        },
        selfBrowserSurface: "include",
        preferCurrentTab:   false,
        systemAudio:        "include",
      }).catch(async (err) => {
        /* Firefox / Safari bazı audio kısıtlamalarını desteklemez, sessiz dene */
      ...
      if (err.name !== "NotAllowedError") {
        console.error("Ekran paylaşımı hatası:", err.name, err.message);
      }
    }
  };

  const stopScreenShare = () => {
    /* screenStreamRef null ise zaten durdurulmuş — çift toggle'ı önle */
    const stream = screenStreamRef.current;
    if (!stream) return;
    screenStreamRef.current = null;  // hemen null yap, onended tekrar çağırırsa guard çalışsın
