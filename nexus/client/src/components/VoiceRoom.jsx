...
function VideoThumb({ stream, muted }) {
  ...
  return <video ref={ref} autoPlay playsInline webkit-playsinline="true" muted={!!muted} preload="auto" className="thumb-video" />;
}
  ...
  */
  ...
  return (
    <div ref={wrapRef} className={`fs-wrap${isMobile ? " fs-mobile" : ""}`}
      ...
      <video ref={videoRef} autoPlay playsInline webkit-playsinline="true" muted={isLocal} preload="auto" className="fs-video" />
...
export default function VoiceRoom({ user, socket, channelId, onLeave }) {
  ...
  const screenStreamRef  = useRef(null); // geç katılan peer'lara ekran track'i eklemek için

  const refreshVideos = useCallback(() =>
    setRemoteVideos(Object.entries(remoteStreamsRef.current).map(([id, s]) => ({ peerId: id, stream: s }))), []);

  /* 1. Mikrofon — yüksek kalite, gürültü giderme */
  useEffect(() => {
  ...
  /* 4. Ekran paylaşımı — 1080p/30fps + düşük gecikme */
    ...
    try {
      const stream = await navigator.mediaDevices.getDisplayMedia({
        video: {
          ...
          cursor:    "always",
          latencyHint: "interactive",   // tarayıcıya "düşük gecikme" iste
        },
      ...
      /* Tarayıcıya hint: görüntü netliği + düşük gecikme dengesi */
      stream.getVideoTracks().forEach((t) => {
        if (t.contentHint !== undefined) t.contentHint = "detail";
      });
      ...
      /* Renegotiation: H264 tercihini de ekle (düşük encode gecikmesi) */
      await Promise.all(
        Object.entries(peersRef.current).map(async ([peerId, pc]) => {
          try {
            ...
            /* Video sender'da H264 codec'i öne al (varsa) */
            const videoSender = pc.getSenders().find((s) => s.track?.kind === "video");
            if (videoSender && RTCRtpSender.getCapabilities) {
