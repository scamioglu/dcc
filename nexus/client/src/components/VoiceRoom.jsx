...
function FullscreenView({ peerId, localScreenStream, remoteStreams, members, volumes, onSetVolume, onClose }) {
  ...
  /* Mobil: orientation değişince stream kopmadan yeniden bağla */
  useEffect(() => {
    if (!isMobile) return;
    const reattach = () => setTimeout(attachStream, 300);
...
export default function VoiceRoom({ user, socket, channelId, onLeave }) {
  ...
  /* 4. Ekran paylaşımı — 60fps + sistem sesi */
  const startScreenShare = async () => {
    try {
      ...
      /* addTrack yeni bir track eklediğinde renegotiation gerekir —
         replaceTrack gerekmez ama addTrack gerektirir. İkisini de kapsamak
         için her stable bağlantıya elle yeni offer gönder. */
      await Promise.all(
